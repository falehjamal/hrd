<div class="row g-3">
    <div class="col-md-6">
        ...
        <select class="form-select @error('deduction_type_id') is-invalid @enderror" id="deduction_type_id" name="deduction_type_id" required>
            ...
            @foreach ($deductionTypes as $type)
                <option value="{{ $type->id }}" @selected(old('deduction_type_id', $deduction->deduction_type_id ?? '') == $type->id)>{{ $type->code }} — {{ $type->name }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-3">
        ...
        <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', $deduction->amount ?? '') }}" min="0" step="1" required />
        @error('amount')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-12">
        <label class="form-label" for="notes">Catatan</label>
        <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2">{{ old('notes', $deduction->notes ?? '') }}</textarea>
        @error('notes')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-12">
        <div class="form-check">
            <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $deduction->is_active ?? true) ? 'checked' : '' }} />
