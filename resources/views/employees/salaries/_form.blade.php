<div class="row g-3">
    ...
    <div class="col-md-4">
        ...
        <input type="number" class="form-control @error('basic_salary') is-invalid @enderror" id="basic_salary" name="basic_salary" value="{{ old('basic_salary', $salary->basic_salary ?? '') }}" min="0" step="1" required />
        @error('basic_salary')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-md-4">
        <label class="form-label" for="fixed_allowance">Tunjangan Tetap (Rp)</label>
        <input type="number" class="form-control @error('fixed_allowance') is-invalid @enderror" id="fixed_allowance" name="fixed_allowance" value="{{ old('fixed_allowance', $salary->fixed_allowance ?? 0) }}" min="0" step="1" />
        @error('fixed_allowance')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-12">
        <label class="form-label" for="notes">Catatan</label>
        <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2">{{ old('notes', $salary->notes ?? '') }}</textarea>
        @error('notes')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-12">
        <div class="form-check">
            <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $salary->is_active ?? true) ? 'checked' : '' }} />
